...
export async function loadTop(category) {
    let top = await loadTemplate('/pages/home/top/top.html');
    top.querySelector('.top-title').textContent = 'Top ' + category;

    let topItems = await fetchJson(`/pages/home/data/${category}.json`);
...
async function loadTopItem(item, topSection) {
    let topItem = await loadTemplate('/pages/home/top/top-item.html')
